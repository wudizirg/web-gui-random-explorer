...
│   ├── state-machine.ts       # 页面状态机 (FSM)
│   ├── form-analyzer.ts       # 表单结构感知
│   ├── llm-strategist.ts      # 增强 LLM 策略层
│   ├── dom-snapshot.ts        # DOM 简化树快照
...
Step N 开始
  ...
  ├─ 2. DomSnapshot.capture()      → DOM 快照 JSON
  ├─ 3. StateMachine.fingerprint() → 页面指纹
  │    └─ StateMachine.classify()  → 页面状态分类
  ├─ 4. FormAnalyzer.analyze()     → 表单结构
  ├─ 5. LlmStrategist.buildPrompt()→ 增强 Prompt
  │    └─ callLlm()                → LLM 决策 (增强)
  ├─ 6. executeAction()            → 执行动作 (已有)
  └─ 7. AnomalyDetector.check()    → 异常检测
       │
